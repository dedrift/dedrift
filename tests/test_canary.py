...
class TestRunner:
    ...
    def test_cycle_shape_and_bookkeeping(self, tmp_path: Path) -> None:
        ...
        per_canary = {
            c: [r.repetition for r in records if r.canary_id == c]
            for c in {r.canary_id for r in records}
        }
        assert all(sorted(reps) == [1, 2, 3] for reps in per_canary.values())  # type: ignore[arg-type]

...
def _rows(scale: str, docs: dict[tuple[str, str], dict[str, Any]]) -> list[dict[str, str]]:
    ...
    return [
        ...
        {
            "name": "Evidently — per canary family, ≥ 1 drifted column",
            "config": "same preset, one report per family",
            "nominal": "none stated",
            "percheck": pc["evidently_family"]["runs_any_drifted_column"],
            "history": None,
        },
        {
        ...
        },
        {
            ...
            "percheck": dd["dedrift_anytime"]["per_fold_alert"],
...
def render_html(docs: dict[tuple[str, str], dict[str, Any]], scale: str) -> str:
    ...
    parts.append(
        ...
        "anytime row's per-check cell is its per-cycle adjudication rate over all 25,000 folds "
        "(500 runs × 50 cycles) — the per-cycle object an e-BH battery produces. Per-run "
...
def _tex_cell(row: dict[str, Any]) -> str:
    """LaTeX table cell: two-line ``k/n`` over ``P% [lo, hi]``.

    Rates below 0.1% get three decimals so a per-fold rate of 0.008% is
    not rounded into indistinguishability from zero.
    """
    tiny = row["rate"] < 0.001
    nd = 3 if tiny else 1
    ...
    return f"\\shortstack{{{k}/{n} \\\\ ${rate}\\%$ $[{lo}, {hi}]$}}"
...
def render_tex(docs: dict[tuple[str, str], dict[str, Any]], scale: str) -> str:
    ...
    lines = [
        ...
        "\\begin{tabular}{@{}p{2.6cm}p{2.9cm}p{2.6cm}cc@{}}",
        "\\toprule",
        "Configuration & Default setting & Nominal / stated & "
        "$\\ge1$ false alarm, per check & "
        "$\\ge1$ false alarm, 50-cycle history \\\\",
        "\\midrule",
    ]
    for row in _rows(scale, docs):
        if "group" in row:
            ...
            lines.append("\\multicolumn{5}{@{}l}{\\textbf{" + _tex_escape(row["group"]) + "}} \\\\")
            lines.append("\\midrule")
            continue
        percheck = _tex_cell(row["percheck"]) if row["percheck"] else "---"
        history = _tex_cell(row["history"]) if row["history"] else "---"
        lines.append(
            f"{_tex_escape(row['name'])} & {_tex_escape(row['config'])} & "
            f"{_tex_escape(row['nominal'])} & {percheck} & {history} \\\\"
        )
...
def render_macros(docs: dict[tuple[str, str], dict[str, Any]]) -> str:
    ...
    for scale, tag in (("suite", "Suite"), ("small", "Small")):
        ...
        out.append(
            _macro(f"benchEvFamAny{tag}", kn(pc["evidently_family"]["runs_any_drifted_column"]))
        )
        out.append(
